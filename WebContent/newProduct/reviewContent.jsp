...
<table id="reviewContent_tb">
	...
	<tr>
		<td colspan="4">
			<textarea readonly><%=dto.getR_content()%></textarea>
		</td>
	</tr>
	<tr>
		<td colspan="4"><input type="button" value="삭제" onclick="location.href='reviewDelete_ok.jsp?idx=<%=dto.getIdx()%>&u_id=<%=u_id%>&r_num=<%=dto.getR_num()%>'"><input type="button" value="수정하기" onclick="location.href='reviewModi.jsp?idx=<%=dto.getIdx()%>&u_id=<%=u_id%>&r_subject=<%=dto.getR_subject()%>&r_star=<%=dto.getR_star()%>&r_content=<%=dto.getR_content()%>&r_num=<%=dto.getR_num()%>'">
